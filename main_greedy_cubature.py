...
def main():
    ...
    def read_training_data(data_dir, data_file=None):
        dataset = []
        if data_file:
            dmat_files = [data_file]
        else:
            dmat_files = [os.path.join(data_dir, f) for f in os.listdir(data_dir) if f.endswith('.dmat')]

        for file_path in dmat_files:
            U = igl.read_dmat(file_path)
            n_vert = U.shape[0]
            dim = U.shape[1]
    ...
    # [N, Nv*3]
    dataset = jnp.array(read_training_data(args.data_dir, args.data_file))
        ...
        while (norm(r) / b_norm > tol) & (n < max_n):
            ...
            if n % args.report_every == 0:
                ...
                if args.save_intermediate:
                    ...
                    # save indices
                    igl.write_dmat(os.path.join(data_dir, f"{args.output_prefix}indices_{n}.dmat"), np.array(selected_set))
                    # save weights
                    igl.write_dmat(os.path.join(data_dir, f"{args.output_prefix}weights_{n}.dmat"), np.array(W))
                    print(f'Write cubature indices and weights to {data_dir}')
                
                with open(data_dir + f'/{args.output_prefix}history.json', 'w') as f:
    ...
    if not os.path.exists(data_dir):
        os.mkdir(data_dir)
    # save indices
    igl.write_dmat(os.path.join(data_dir, f"{args.output_prefix}indices.dmat"), np.array(selected_set))
    # save weights
    igl.write_dmat(os.path.join(data_dir, f"{args.output_prefix}weights.dmat"), np.array(W))
    print(f'Write cubature indices and weights to {data_dir}')
